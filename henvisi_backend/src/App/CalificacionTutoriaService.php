...
use Src\App\model\DetalleCalificacionTutoria;
use Src\App\model\TipoVisualizacion;

class CalificacionTutoriaService
{
     ...
     */
    public function storeCalificacion(DetalleCalificacionTutoria $detalleCalificacion)
    {
        try {
            DB::beginTransaction();
            $docente = Auth::user()->docente; // Usa findOrFail para manejar mejor el caso de no encontrar la materia
            $nuevasCalificaciones = [];
            $calificacionExistente = $docente->calificaciones()
            ...
            is_null($calificacionExistente) ? $docente->calificaciones()->createMany($nuevasCalificaciones) : $this->actualizarCalificacion($nuevasCalificaciones, $docente);
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
     ...
     */
    private function actualizarCalificacion($calificaciones, Docente $docente)
    {
        try {
            DB::beginTransaction();
            foreach ($calificaciones as $calificacion) {
                $data = [
                    'calificacion_cuantitativa' => $calificacion['calificacion_cuantitativa'],
                    'calificacion_cualitativa' => $calificacion['calificacion_cualitativa'],
                ];
                // Buscar si ya existe la calificación
                $exists = $docente->calificaciones()
        ...
        foreach ($calificacion as $key => $value) {
            if (is_numeric($key)) {
                $result[] = [
                    ...
                    'materia_id' => $key,
                    'calificacion_cuantitativa' => $value,
                    'trimestre' => $trimestre,
                ];
            }
        }
    ...
    public function obtenerCalificacionesMapeada(CalificacionMapeada $calificacionMap)
    {
        $materias = Materia::where('id', $calificacionMap->materia)
            ->with(['calificaciones.estudiante' => function ($query) {
                ...
                $query->select('id', 'nombre', 'insumo_id'); // Seleccionar solo los campos necesarios de la actividad
            }])
            ->get();
